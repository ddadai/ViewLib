...
import com.wheel.widget.adapters.TimeWheelAdapter;
...
public class TimeWheelPopView extends BaseWheelPopView{
	...
	protected TimeWheelPopView(Activity context,int timeType,int minYear,int maxYear,boolean is24Hour) {
		super(context);
	...
	public TimeWheelPopView(Activity context,int timeType,int minYear,int maxYear) {
		this(context, timeType,minYear,maxYear,true);
	}
	
	
	public TimeWheelPopView(Activity context,int timeType) {
	...
	public void setYear(int minYear,int maxYear){
		...
		if(currentYear<this.maxYear||currentYear>this.maxYear){
