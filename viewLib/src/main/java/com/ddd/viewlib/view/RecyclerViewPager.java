...
import com.ddd.viewlib.BuildConfig;
import com.ddd.viewlib.R;
import com.ddd.viewlib.adapter.RecyclerViewPagerAdapter;
import com.ddd.viewlib.utils.RecyclerViewUitl;
...
public class RecyclerViewPager extends RecyclerView {
   ...
   protected int mMaxLeftWhenDragging = Integer.MIN_VALUE;
    protected  int mMinLeftWhenDragging = Integer.MAX_VALUE;
    protected int mMaxTopWhenDragging = Integer.MIN_VALUE;
    protected int mMinTopWhenDragging = Integer.MAX_VALUE;
...
//        setNestedScrollingEnabled(false);
    ...
    public int getCurrentPosition() {
        int curPosition;
        if (getLayoutManager().canScrollHorizontally()) {
            curPosition = RecyclerViewUitl.getCenterXChildPosition(this);
        } else {
            curPosition = RecyclerViewUitl.getCenterYChildPosition(this);
        }
        if (curPosition < 0) {
    ...
    protected void adjustPositionX(int velocityX) {
        ...
        if (childCount > 0) {
            int curPosition = RecyclerViewUitl.getCenterXChildPosition(this);
            ...
            if (targetPosition == curPosition
                ...
                View centerXChild = RecyclerViewUitl.getCenterXChild(this);
    ...
    protected void adjustPositionY(int velocityY) {
        ...
        int childCount = getChildCount();
        if (childCount > 0) {
            int curPosition = RecyclerViewUitl.getCenterYChildPosition(this);
            ...
            if (targetPosition == curPosition
                    && (!mSinglePageFling || mPositionOnTouchDown == curPosition)) {
                View centerYChild = RecyclerViewUitl.getCenterYChild(this);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN && getLayoutManager() != null) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally()
                    ? RecyclerViewUitl.getCenterXChildPosition(this)
                    : RecyclerViewUitl.getCenterYChildPosition(this);
            if (DEBUG) {
...
//            }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? RecyclerViewUitl.getCenterXChild(this) :
                    RecyclerViewUitl.getCenterYChild(this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? RecyclerViewUitl.getCenterXChildPosition(this) :
                        RecyclerViewUitl.getCenterYChildPosition(this);
                if (mCurView != null) {
