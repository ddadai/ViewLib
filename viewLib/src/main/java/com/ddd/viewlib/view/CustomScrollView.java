 ...
 * 
 */
public class CustomScrollView extends FrameLayout implements
...
//		setOrientation(VERTICAL);
		...
		FrameLayout.LayoutParams headParams = new FrameLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT);
		headParams.topMargin = -customHeadView.getMeasuredHeight();
		headParams.gravity=Gravity.TOP;
		headHeight=customHeadView.getMeasuredHeight();
		...
		FrameLayout.LayoutParams footParams = new FrameLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,customFootView.getMeasuredHeight());
		footHeight=customFootView.getMeasuredHeight();
	...
	public void ScrollChange(int l, int t, int oldl, int oldt) {
		if(onScrollChangeListenerl!=null){
			onScrollChangeListenerl.ScrollChange(l, t, oldl, oldt);
		}
		if(downPullListener!=null){
			downPullListener.pullMove(l, t);
		}
	}
	...
	public void setOnRefreshHeadListener(
			OnRefreshHeadListener onRefreshHeadListener) {
	...
	public void setOnRefreshFooterListener(
			OnRefreshFooterListener onRefreshFooterListener) {
	...
	public void setOnScrollChangeListenerl(
			OnScrollChangeListener onScrollChangeListenerl) {
...
//			else if (child instanceof CordovaWebView) {
//				cordovaWebView = (CordovaWebView) child;
//			}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (offset > 0) {// 向上拉
				...
				if (overWriteScrollView != null) {
					...
					interceptTouch = listView.getChildCount() == 0 ? true: listView.getLastVisiblePosition() == listView.getCount() - 1&& listView.getChildAt(listView.getChildCount() - 1).getBottom() <= listView.getHeight();
				} else if (recyclerView != null&& recyclerView.getVisibility() == View.VISIBLE) {
					
					
					interceptTouch = recyclerView.getChildCount() == 0 ? true:
//						recyclerView.getChildPosition() == recyclerView.getChildCount() - 1&& 
						(recyclerView.getChildAt(recyclerView.getChildCount() - 1).getBottom() <= recyclerView.getHeight());
//					interceptTouch = ((AbsListView) listView).getChildCount() == 0 ? true: listView.getLastVisiblePosition() == listView.getCount() - 1&& ((AbsListView) listView).getChildAt(((AbsListView) listView).getChildCount() - 1).getBottom() <= ((AbsListView) listView).getHeight();
				}
				...
				if (overWriteScrollView != null) {
					interceptTouch = overWriteScrollView.getScrollY() <= 0;// 判断滚动到最顶部
						...
						&& expandableListView.getVisibility() == View.VISIBLE) {
					interceptTouch = expandableListView
							.getFirstVisiblePosition() == 0;
				} else if (listView != null && listView.getVisibility() == View.VISIBLE) {
					interceptTouch = listView.getFirstVisiblePosition() == 0&& listView.getChildCount() == 0 ? true: listView.getChildAt(0).getTop() == 0;
				} else if (recyclerView != null && recyclerView.getVisibility() == View.VISIBLE) {
					interceptTouch =
//							recyclerView.getChildPosition(getChildAt(index)) == 0&&
							recyclerView.getChildCount() == 0 ? true: recyclerView.getChildAt(0).getTop() == 0;
				} 
				else if (webView != null) {
					interceptTouch = webView.getScrollY() <= 0;
				} 
//				else if (cordovaWebView != null) {
//					interceptTouch = cordovaWebView.getScrollY() <= 0;
//				}
		...
		return interceptTouch && endablePull && Math.abs(offset) > 10;
	}
		...
		if (lock) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (offset > 0) {
					// 向上拉
					directionXY = true;
					if (Math.abs(getScrollY()) < headHeight) {
				...
				if(offset<-10&&downPullListener!=null){
					downPullListener.pullStart();
				}
				...
				} else if (pullStatus.equals(Status.up.toString())
						&& !directionXY) {
					if (getScrollY() <= 0) {
						return false;
					}
	...
	public void complatePull() {
		...
		if (directionXY) {
			...
			if (downPullListener != null) {
				downPullListener.pullEnd();
			}
		} else {
			if (this.headView&& Math.abs(getScrollY()) >= headHeight) {
				refreshDownPull();
			} else {
				pullDown();
	...
	public void refreshDownPull() {
		...
		interceptTouch = true;
		if (Math.abs(getScrollY()) >= headHeight) {
			scroller.startScroll(0,getScrollY(),	0,Math.abs(getScrollY() + headHeight),500);
	...
	public void refreshUpPull() {
		interceptTouch = true;
		if (Math.abs(getScrollY()) >= footHeight) {
			...
			scroller.startScroll(0, getScrollY(), 0,
					-(Math.abs(getScrollY()) - footHeight), 500);
		} else {
			pullUp();
		}
	...
	private OnDownPullListener downPullListener;
	public void setOnDownPullListener(OnDownPullListener downPullListener){
		this.downPullListener=downPullListener;
	}
	public interface OnDownPullListener{
		void pullStart();
		void pullEnd();
		void pullMove(int x,int y);
	}
}
